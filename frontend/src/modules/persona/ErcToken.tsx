...
import { RiCopperCoinFill, RiDeleteBin7Line } from "react-icons/ri";
...
const ErcToken = ({
  ...
  //-------Function to search for a ercToken-------//
  const handleSearch = async (searchValue: string, searchEndpoint: string) => {
    if (searchEndpoint) {
      ...
      const filteredOptions = res.data.data.map((erc20: any) => {
        return {
          value: erc20.contract_address,
          label: erc20.name,
          imageUrl: erc20.image_link || "/images/assets/default.png",
        };
      });
  ...
  const updateBehavior = (option?: any, subOption?: any, subOption2?: any) => {
    ...
    // console.log("new filter", option, subOption, subOption2);
  ...
  //-------Function to update ercToken-------//
  const updateErcToken = (option: any) => {
    let tempPersonaData = personaData;
    tempPersonaData.blocks[blockId].data.filters[filterId].filterData.token = {
      ...option,
    };
  ...
  return (
    <Card>
      ...
      <Dropdown
        ...
        style={{ width: "200px" }}
        onChange={updateErcToken}
        selectedChoice={filter?.filterData?.collection?.label}
        searchEndpoint={API_URL + "/filters/erc20/search"}
